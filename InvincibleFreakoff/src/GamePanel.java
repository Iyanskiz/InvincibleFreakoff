...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    private void runBotAI(Fighter bot, Fighter target, int timer) {
        ...
        // ── Natural jumping ───────────────────────────────────────────────
        if (jmpCool == 0 && rand.nextInt(100) < (botDifficulty==0?2:botDifficulty==1?5:8)) {
            bot.jump();
    ...
    private void updateStageEvents() {
        switch(currentStage) {
            ...
            case 4: // Volcano — lava embers constantly rising
                if(tick%2==0){
                    int lx=rand.nextInt(1280);
                    particles.add(new Particle(lx,Fighter.GROUND_Y+130,(rand.nextFloat()-0.5f)*3,-rand.nextFloat()*5-2,
            ...
            case 5: // Viltrumite Throne Room — crackling energy orbs
                if(rand.nextInt(80)==0) energyOrbs.add(new EnergyOrb(rand.nextInt(1280),200+rand.nextInt(300)));
                if(tick%8==0){
                    Color gc=rand.nextInt(2)==0?new Color(255,180,0,120):new Color(200,0,0,100);
                    particles.add(new Particle(rand.nextInt(1280),Fighter.GROUND_Y+130,(rand.nextFloat()-0.5f)*1.5f,-rand.nextFloat()*2-0.5f,gc,50+rand.nextInt(30)));
                }
                break;
            case 6: // Antarctic — blizzard snow particles
                for(int i=0;i<4;i++){
                    particles.add(new Particle(rand.nextInt(1280),-5,
                ...
                if(lightningTimer>120+rand.nextInt(200)){
                    lightningTimer=0;
                    int lx=rand.nextInt(1280);
                    lightning.add(new LightningBolt(lx,0,lx+(rand.nextInt(60)-30),Fighter.GROUND_Y+130));
    ...
    protected void paintComponent(Graphics g0) {
        ...
        if(p1!=null) { if(!p1.isDead()) p1.draw(g); else if(gameMode==1||gameMode==3) drawTombstone(g, p1.x, Fighter.GROUND_Y, p1.name, p1.facingRight); }
        if(p2!=null) { if(!p2.isDead()) p2.draw(g); else if(gameMode==1||gameMode==3) drawTombstone(g, p2.x, Fighter.GROUND_Y, p2.name, p2.facingRight); }
        if(p1b!=null) { if(!p1b.isDead()) p1b.draw(g); else drawTombstone(g, p1b.x, Fighter.GROUND_Y, p1b.name, p1b.facingRight); }
        if(p2b!=null) { if(!p2b.isDead()) p2b.draw(g); else drawTombstone(g, p2b.x, Fighter.GROUND_Y, p2b.name, p2b.facingRight); }
    ...
    private void drawVolcano(Graphics2D g, int W, int H) {
        ...
        int[] vx={0,0,120,180,300,360,480,560,680,760,880,940,1060,1120,1240,1280,1280};
        int[] vy={H,460,460,390,390,430,430,370,370,415,415,365,365,400,400,440,H};
        g.fillPolygon(vx,vy,vx.length);
        // Glowing lava vents in the volcano cones
        int[] ventX={180,360,560,760,940,1120};
        for(int vnt:ventX){
        ...
        // Lava river floor
        GradientPaint lavaPaint=new GradientPaint(0,Fighter.GROUND_Y+130,new Color(180,40,0),0,H,new Color(60,10,0));
        g.setPaint(lavaPaint); g.fillRect(0,Fighter.GROUND_Y+130,W,H-Fighter.GROUND_Y-130);
        // Lava glow ripple
        float lavaGlow=(float)(Math.sin(tick*0.04)*0.3+0.7);
        g.setColor(new Color(255,100,0,(int)(lavaGlow*90)));
        g.setStroke(new BasicStroke(4));
        g.drawLine(0,Fighter.GROUND_Y+130,W,Fighter.GROUND_Y+130);
        g.setStroke(new BasicStroke(1));
        // Lava pools
        for(int i=0;i<6;i++){
            int lx=80+i*190; float ph=(float)(Math.sin(tick*0.05+i)*0.25+0.75);
    ...
    private void drawThroneRoom(Graphics2D g, int W, int H) {
        ...
        // Stone floor
        GradientPaint floor=new GradientPaint(0,Fighter.GROUND_Y+130,new Color(25,8,5),0,H,new Color(10,3,2));
        g.setPaint(floor); g.fillRect(0,Fighter.GROUND_Y+130,W,H-Fighter.GROUND_Y-130);
        // Floor tiles
        g.setColor(new Color(35,12,8,100));
        for(int fx=0;fx<W;fx+=80) g.drawLine(fx,Fighter.GROUND_Y+130,fx,H);
        for(int fy=Fighter.GROUND_Y+130;fy<H;fy+=50) g.drawLine(0,fy,W,fy);
        // Gold ground line
    ...
    private void drawAntarctic(Graphics2D g, int W, int H) {
        ...
        int[]icx={0,0,60,60,130,130,200,200,280,280,370,370,460,460,560,560,650,650,740,740,830,830,920,920,1010,1010,1100,1100,1190,1190,1280,1280};
        int[]icy={H,400,400,370,370,392,392,355,355,380,380,348,348,375,375,340,340,368,368,338,338,362,362,345,345,368,368,340,340,360,360,H};
        ...
        // Icy frozen ground
        GradientPaint icePaint=new GradientPaint(0,Fighter.GROUND_Y+130,new Color(30,55,90),0,H,new Color(10,20,40));
        g.setPaint(icePaint); g.fillRect(0,Fighter.GROUND_Y+130,W,H-Fighter.GROUND_Y-130);
        // Ice crack patterns
        g.setColor(new Color(80,140,200,60));
        g.setStroke(new BasicStroke(1));
        for(int i=0;i<8;i++){
            int cx=50+i*160;
            g.drawLine(cx,Fighter.GROUND_Y+130,cx+40,Fighter.GROUND_Y+160);
    ...
    private void drawHUD(Graphics2D g,int W,int H){
        ...
        if(gameMode==0||gameMode==2){
            g.drawString("P1: WASD + F/G/H",30,topY+46);
            g.drawString("P2: Arrows + NP1/2/3",W-190,topY+46);
        } else {
            // 2v2 - show team labels clearly
            ...
            g.drawString("TEAM 2",W-115,topY+36);
            g.setColor(new Color(255,255,255,55));
            g.drawString("P2: Arrows+NP123",W-160,topY+47);
            g.drawString("P4: NP4568+790",W-148,topY+58);
        }
        // Bot difficulty tag
    ...
    private void drawTombstone(Graphics2D g, int x, int groundY, String name, boolean facingRight) {
