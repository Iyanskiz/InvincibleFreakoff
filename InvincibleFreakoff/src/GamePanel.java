...
public class GamePanel extends JPanel implements ActionListener, KeyListener {
    ...
    public GamePanel(int[] p1Indices, int[] p2Indices, int gameMode, int[] botIndices, int botDifficulty) {
        ...
        // Placeholder positions — reposition() is called every frame in drawHUD
        hb1  = new HealthBar(0, 0, 100, 28, true);
        hb2  = new HealthBar(0, 0, 100, 28, false);
        hb1b = new HealthBar(0, 0, 100, 20, true);
        hb2b = new HealthBar(0, 0, 100, 20, false);
    ...
    private void update() {
        ...
        buildingDebris.removeIf(b -> b.y > 750);
        spaceDebris.removeIf(s -> s.x > WORLD_W + 20);
    ...
    private void checkPair(Fighter a, Fighter d) {
        ...
        if (hit && !d.isHurt) {
            ...
            a.chargeSpecial(a.attackDamage);
    ...
    private void updateStageEvents() {
        switch(currentStage) {
            case 0:
                meteorTimer++;
                if(meteorTimer>=meteorInterval){meteorTimer=0;meteorInterval=120+rand.nextInt(180);meteors.add(new Meteor(100+rand.nextInt(WORLD_W-200)));}
                if(tick%3==0) particles.add(new Particle(rand.nextInt(WORLD_W),GROUND_Y+150,(rand.nextFloat()-0.5f)*2,-rand.nextFloat()*3-1,new Color(255,80+rand.nextInt(100),0,200),30+rand.nextInt(20)));
                break;
            case 1:
                for(SpaceDebris s:spaceDebris) s.x-=0.5f;
                if(rand.nextInt(60)==0) spaceDebris.add(new SpaceDebris(WORLD_W+20,rand.nextInt(450)));
                break;
            case 2:
                lightningTimer++;
                if(lightningTimer>60+rand.nextInt(120)){
                    lightningTimer=0; thunderFlash=true;
                    int lx=rand.nextInt(WORLD_W);
                ...
                break;
            case 3:
                if(rand.nextInt(120)==0) energyOrbs.add(new EnergyOrb(rand.nextInt(WORLD_W),200+rand.nextInt(350)));
                if(tick%5==0) particles.add(new Particle(rand.nextInt(WORLD_W),rand.nextInt(VIEW_H),(rand.nextFloat()-0.5f),(rand.nextFloat()-0.5f),new Color(150,0,255,Math.max(0,Math.min(255,100+rand.nextInt(100)))),40+rand.nextInt(30)));
                break;
            case 4:
                if(tick%2==0){
                    int lx=rand.nextInt(WORLD_W);
                ...
                if(meteorTimer>=meteorInterval){meteorTimer=0;meteorInterval=90+rand.nextInt(120);
                    ...
                    meteors.add(new Meteor(WORLD_W-200+rand.nextInt(200)));
                }
                break;
            case 5:
                if(rand.nextInt(80)==0) energyOrbs.add(new EnergyOrb(rand.nextInt(WORLD_W),200+rand.nextInt(350)));
                if(tick%8==0){
                    Color gc=rand.nextInt(2)==0?new Color(255,180,0,120):new Color(200,0,0,100);
                    particles.add(new Particle(rand.nextInt(WORLD_W),GROUND_Y+150,(rand.nextFloat()-0.5f)*1.5f,-rand.nextFloat()*2-0.5f,gc,50+rand.nextInt(30)));
                }
                break;
            case 6:
                for(int i=0;i<4;i++){
                    particles.add(new Particle(rand.nextInt(WORLD_W),-5,
    ...
    protected void paintComponent(Graphics g0) {
        ...
        AffineTransform screenBase = g.getTransform();
    ...
    private void drawBackground(Graphics2D g, int W, int H) {
        AffineTransform bgBase = g.getTransform();
        g.translate(camTX, camTY);
        g.scale(camScale, camScale);
        switch(currentStage) {
            case 0: drawCity(g, WORLD_W, H);       break;
            case 1: drawSpace(g, WORLD_W, H);      break;
            case 2: drawStorm(g, WORLD_W, H);      break;
    ...
    private void drawVolcano(Graphics2D g, int W, int H) {
        ...
        for(int i=0;i<5;i++){
            ...
            for(int cx=-200;cx<W+200;cx+=280){int cloudX=(cx+offset)%(W+400)-200,cloudY=20+i*30;g.fillOval(cloudX,cloudY,310,90);g.fillOval(cloudX+70,cloudY-24,190,70);}
        }
        ...
        for(int vnt:ventX){
            RadialGradientPaint lava=new RadialGradientPaint(vnt,390,40,new float[]{0f,0.5f,1f},new Color[]{new Color(255,120,0,200),new Color(255,50,0,80),new Color(0,0,0,0)});
        ...
        g.setColor(new Color(255,100,0,(int)(lavaGlow*90))); g.setStroke(new BasicStroke(4));
        g.drawLine(0,GROUND_Y+150,W,GROUND_Y+150); g.setStroke(new BasicStroke(1));
        for(int i=0;i<9;i++){
            int lx=80+i*170; float ph=(float)(Math.sin(tick*0.05+i)*0.25+0.75);
            g.setColor(new Color(255,80,0,(int)(ph*130))); g.fillOval(lx,GROUND_Y+138,100,22);
            g.setColor(new Color(255,200,50,(int)(ph*80))); g.fillOval(lx+20,GROUND_Y+140,60,12);
        }
    }
    ...
    private void drawThroneRoom(Graphics2D g, int W, int H) {
        ...
        for(int i=0;i<13;i++){
            int cx=i*130;
            g.setColor(new Color(22,8,6,200)); g.fillRect(cx,0,10,GROUND_Y+150);
            g.setColor(new Color(35,12,8,100)); g.fillRect(cx+2,0,6,GROUND_Y+150);
        }
        ...
        for(int i=0;i<7;i++){
            ...
            g.setColor(new Color(220,170,0)); g.setStroke(new BasicStroke(2)); g.drawLine(bx-2,by-10,bx-2,by+110); g.setStroke(new BasicStroke(1));
            g.setColor(new Color(255,210,0,200)); g.drawLine(bx+10,by+30,bx+20,by+55); g.drawLine(bx+20,by+55,bx+30,by+30);
        }
        g.setColor(new Color(30,10,8)); g.fillRect(550,280,180,220);
        g.setColor(new Color(50,18,12)); g.fillRect(560,240,160,240);
        g.setColor(new Color(200,150,0)); g.setStroke(new BasicStroke(3)); g.drawRect(560,240,160,240); g.drawRect(566,246,148,228); g.setStroke(new BasicStroke(1));
        g.setColor(new Color(255,200,0,80)); g.fillRect(560,240,160,15);
        for(int i=0;i<8;i++){
            int tx=60+i*210,ty=200;
            float ph=(float)(Math.sin(tick*0.07+i)*0.4+0.6);
            RadialGradientPaint torch=new RadialGradientPaint(tx,ty,50,new float[]{0f,0.5f,1f},new Color[]{new Color(255,150,0,(int)(ph*200)),new Color(255,60,0,(int)(ph*80)),new Color(0,0,0,0)});
        ...
        g.setColor(new Color(200,140,0,180)); g.setStroke(new BasicStroke(3)); g.drawLine(0,GROUND_Y+150,W,GROUND_Y+150); g.setStroke(new BasicStroke(1));
    }
    ...
    private void drawAntarctic(Graphics2D g, int W, int H) {
        ...
        for(int i=0;i<4;i++){
            ...
            g.setColor(aurora); g.setStroke(new BasicStroke(22+i*8));
            g.drawLine(0,ay,W,ay+(int)(Math.sin(tick*0.01+i)*30)); g.setStroke(new BasicStroke(1));
        }
        ...
        for(int i=0;i<icx.length-2;i+=2){if(icy[i]<420) g.fillRect(icx[i],icy[i],Math.max(1,icx[i+1]-icx[i]),4);}
        ...
        g.setColor(new Color(80,140,200,60)); g.setStroke(new BasicStroke(1));
        for(int i=0;i<11;i++){int cx=50+i*145;g.drawLine(cx,GROUND_Y+150,cx+40,GROUND_Y+160);g.drawLine(cx+40,GROUND_Y+160,cx+10,GROUND_Y+180);g.drawLine(cx+40,GROUND_Y+160,cx+70,GROUND_Y+185);}
        g.setColor(new Color(140,200,255,200)); g.setStroke(new BasicStroke(3)); g.drawLine(0,GROUND_Y+150,W,GROUND_Y+150);
    ...
    private void drawHUD(Graphics2D g, int W, int H) {
        ...
        if ((gameMode == 1 || gameMode == 3) && p1b != null && p2b != null) {
            hb1b.reposition(margin, barY2, barWidth);
            hb2b.reposition(W / 2 + timerW, barY2, barWidth);
        ...
        if (roundTimer <= 10) {
            int a = Math.max(0, Math.min(255, 150 + (int)(Math.sin(tick * 0.3) * 105)));
            tc = new Color(255, 0, 0, a);
        }
        ...
        for (int i = 0; i < 2; i++) {
            int px = margin + i * 22;
            if (i < p1Wins) { g.setColor(new Color(255,220,50)); g.fillOval(px, pipY, 16, 16); }
            else             { g.setColor(new Color(60,60,60));  g.drawOval(px, pipY, 16, 16); }
        }
        for (int i = 0; i < 2; i++) {
            int px = W - margin - 36 + i * 22;
            if (i < p2Wins) { g.setColor(new Color(255,220,50)); g.fillOval(px, pipY, 16, 16); }
            else             { g.setColor(new Color(60,60,60));  g.drawOval(px, pipY, 16, 16); }
        }
        ...
        if (p1 != null && p1.isBlocking) {
            ...
            g.drawString("BLOCKING", margin, infoY + 14);
        }
        ...
        if (gameMode == 0 || gameMode == 2) {
            g.drawString("P1: WASD+F/G/H  |  DOWN+JUMP=Drop", margin, infoY + 28);
            g.drawString("P2: Arrows+NP1/2/3  |  DOWN+JUMP=Drop", W - 265, infoY + 28);
        } else {
            g.setColor(new Color(50,150,255,150)); g.drawString("TEAM 1", margin, infoY + 20);
            g.setColor(new Color(255,255,255,55));
            g.drawString("P1: WASD+FGH", margin, infoY + 31);
            g.drawString("P3: IJKL+UOP", margin, infoY + 42);
            g.setColor(new Color(255,80,80,150)); g.drawString("TEAM 2", W - 135, infoY + 20);
            g.setColor(new Color(255,255,255,55));
            g.drawString("P2: Arrows+NP123", W - 185, infoY + 31);
    ...
    private void drawTombstone(Graphics2D g, int x, int groundY, String name) {
        ...
        g.setColor(new Color(30,30,30)); g.fillRoundRect(tx+15,ty+70,60,20,4,4);
        g.setColor(new Color(80,80,88)); g.fillRoundRect(tx+20,ty+10,52,65,10,10);
        g.setColor(new Color(100,100,110)); g.fillRoundRect(tx+22,ty+12,48,62,9,9);
        g.setColor(new Color(90,90,98)); g.fillArc(tx+20,ty,52,30,0,180);
        g.setColor(new Color(105,105,115)); g.fillArc(tx+22,ty+2,48,26,0,180);
        g.setColor(new Color(55,55,62)); g.fillRect(tx+43,ty+20,6,30); g.fillRect(tx+33,ty+28,26,6);
        g.setFont(new Font("Impact",Font.PLAIN,10)); g.setColor(new Color(40,40,44));
        FontMetrics fm=g.getFontMetrics(); String rip="R.I.P";
        ...
        g.setColor(new Color(50,50,55)); g.drawString(shortName,tx+46-fm.stringWidth(shortName)/2,ty+66);
        g.setColor(new Color(0,0,0,55)); g.fillOval(tx+10,groundY+5,72,12);
    }
